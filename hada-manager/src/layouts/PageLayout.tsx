...
export function Layout() {
  ...
  useEffect(() => {
    setRooms([{id:"123456", name:"חד\"א קצינים שיש לו עכשיו שם ממש אבל ממש אבל מממממששששששש ארוך"},
	...
	const SeatPerson = (person:PersonType, table:TableType) => {
      ...
      setTables(prev => prev.map(_ => _.id === otherContainingTable.id ? {..._, peopleIds:_.peopleIds.filter( pid => pid !== person.id)} : _));
